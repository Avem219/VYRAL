...
- **Media**: select one of the already-implemented production providers in `src/lib/storage.ts`
  (S3, R2, or Supabase) and set `STORAGE_PROVIDER` plus its credentials. The local-disk provider is dev-only.
...
| Media: thumbnailing, image optimization, orphan cleanup | 🟡 | Core storage and authorization are complete; advanced optimization/orphan-garbage-collection is operational hardening rather than a launch blocker. |
...
| Moderation/Admin | ✅ | Reports, moderator/admin queue, suspension, audit logging, moderation UI, and direct removal of reported post/reel/story/comment content are implemented and role-gated server-side. |
